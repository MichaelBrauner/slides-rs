...
```
CLI Input → Configuration Loading → Template Rendering → Asset Copying → Thumbnail Generation → Output
```

### Core Modules

1. **CLI Layer** - Command parsing and user interaction (clap)
2. **Configuration** - Loading and validating decks.yaml and translations
3. **Rendering Engine** - Template processing with MiniJinja
4. **Asset Pipeline** - Static file handling and optimization
5. **Browser Integration** - Headless Chrome orchestration for thumbnails and PDF
